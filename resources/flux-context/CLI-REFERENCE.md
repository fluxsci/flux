...
| `zotero-sync [--bib f] [--data-dir d] [--attach copy\|link] [--save]` | `zotero_sync` | pull new references + PDFs from the connected Zotero Better-BibTeX auto-export into FluxLib (one-way, idempotent; `--save` persists overrides as the machine settings) |
