...
const CLOSING = /\$\$\s*(\{#eq-[A-Za-z0-9_-]+\}\s*)?([.,;:!?])?$/;
const CLOSES = (t: string): boolean => CLOSING.test(t);
...
export class MathBlockTracker {
  ...
  feed(lineNo: number, raw: string): MathBlock | null {
    ...
    if (CLOSES(rest)) {
      // Single-line $$…$$ [{#eq-id}]
      const label = EQ_LABEL_TAIL.exec(t)?.[1];
      const tex = rest.replace(CLOSING, "").trim();
      return { startLine: lineNo, endLine: lineNo, tex, label, suffix: CLOSING.exec(rest)?.[2] };
    }
    this.open = true;
    this.start = lineNo;
