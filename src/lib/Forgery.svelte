<script lang="ts">
  ...
  import { forgeryOpen, settings } from "./settings";
  ...
  $: fields = $forgeryOpen ? buildFields($project, $selection) : [];
  ...
  // Reset state each time the Forgery opens.
  let prevOpen = false;
  $: {
    if ($forgeryOpen && !prevOpen) reset();
    prevOpen = $forgeryOpen;
  }
  function reset() {
    mode = "hotkey";
    activeKey = null;
  ...
  const R3 = 14; // panel corner radius (matches --r-3)
  function halfFrame(w: number, h: number, right: boolean, inset = 1.4): string {
    ...
    const cx = w / 2;
    // Each half starts at the top-centre and ends at the bottom-centre, mirrored,
    // so the two strokes meet (seal) at the bottom as --draw reaches 1.
    return right
  ...
  function forge(_node: HTMLElement) {
    ...
    // "quick-fade": the whole panel (frame already drawn at rest) fades in/out
    // together, very fast. No line draw.
    if (get(settings).forgeryAnim === "fade") {
      return {
        duration: 105,
        css: (t: number) => `opacity:${t}; transform: scale(${0.985 + 0.015 * t});`,
      };
    }
    // "draw": the accent frame draws itself, then content rises — fully in (or
    // back out) within ~180ms so it feels instant.
  ...
  function onWin(e: KeyboardEvent) {
    if (!$forgeryOpen || mode !== "hotkey") return;
    const k = e.key;
  ...
  $: width = { sm: 420, md: 560, lg: 720 }[$settings.forgerySize];
  $: wrapStyle = {
  ...
  $: bgAlpha = $settings.forgeryOpacity;
