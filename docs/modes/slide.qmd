...
**Alt+I** opens the same [Plot gallery](figure.qmd#getting-plots-in) used by Figure.
...
A **Ghost transform…** creates independent copies that move out from an object. For example,
...
1. Select the source object and the step where the copies should appear, then choose
   **Ghost transform…** in the animation pane.
...
3. Create the ghosts. Flux selects the first copy in **Edit after step**. Move, resize,
   rotate, restyle, or edit it with the ordinary tools to set its destination.
...
Keep **`.mp4` and `.mov`** files in **`plots/_videos`**. Open **Plots & videos** in Slide mode (or press **Alt+I**),
...
delay. Animation durations, easing, plot morphs, ghost copies, text changes and camera
