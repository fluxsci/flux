...
> fresh clone of this repo on a user's Mac. It is deliberately **not** part of the rendered
> user documentation (`_quarto.yml` renders `.qmd` only; this is a plain `.md`). The
...
> say something like *"Read docs/claude-install-flux-mac.md and set up Flux on this Mac."*
...
# Generated by the Flux install runbook (docs/claude-install-flux-mac.md).
set -euo pipefail
