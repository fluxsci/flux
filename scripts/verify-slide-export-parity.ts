...
//  2. bare-assetId content targets (a data-only Become) resolve via the plots/<id>.svg convention.
...
try {
  ...
  // a data-only Become to a plot that is NOT on any slide — bare assetId reference
  slideOps.setAnimation(deck, sid, beat.id, {
    id: "t2", target: "el-plot", preset: "transform", duration: 1200,
    to: { assetId: "example_plots/08_ecdf", state: {} },
  } as Track);
  await slides.saveDeck(root, deck);

  // --- 1+2: gather resolves manifest via sibling + content target via convention
  ...
  assert(!!ecdf?.svg, "bare-assetId content target gathered via plots/<id>.svg convention");
  assert(!!(ecdf.manifest as unknown as { parts?: unknown }).parts, "content target manifest gathered via sibling");
  ...
  // restore, then break the content target's svg
  ...
  assert(
    ...
    "missing content target svg → warning",
  );
