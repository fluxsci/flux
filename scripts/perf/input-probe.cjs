...
if (!process.versions.electron) {
  ...
  const env = { ...process.env, HOME: path.join(scratch, 'home'), XDG_CONFIG_HOME: path.join(scratch, 'xdg'), APPDATA: path.join(scratch, 'appdata'), FLUX_NO_MIGRATE: '1',
    PROBE_PROJECT: project, PROBE_OUT: out, PROBE_SCENARIOS: opt('scenarios', 'base'), PROBE_SURFACE: opt('surface', 'figure'), PROBE_PHASES: opt('phases', ''), PROBE_TRACE: args.includes('--trace') ? '1' : '0', PROBE_FRAMES: args.includes('--frames') ? '1' : '0' };
  ...
  child.on('close', (code) => {
    ...
    console.log(`input-probe: exit=${code} out=${out}`);
    process.exit(code ?? 1);
  });
  return;
}
...
function analyzeFrames(frames) {
  ...
  const blanks = [], flashes = [], reversals = [];
  for (let i = 1; i < frames.length - 1; i++) {
    ...
    if (f.canvasContent < Math.min(p.canvasContent, n.canvasContent) * 0.4 && p.canvasContent > med * 0.3) blanks.push({ i, t: f.t - frames[0].t, content: f.canvasContent, prev: p.canvasContent, next: n.canvasContent });
...
async function measureFrames(label, region, run, traceName) {
  ...
  const onMsg = (_e, method, params) => {
    ...
    try { const img = nativeImage.createFromBuffer(Buffer.from(params.data, 'base64')); const sz = img.getSize(); frames.push(frameStats(img, region, { x: sz.width / region.winW, y: sz.height / region.winH })); } catch {}
  ...
  void scale;
  res.framesOracle = analyzeFrames(frames);
