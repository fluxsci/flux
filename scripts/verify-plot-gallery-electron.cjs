...
async function main(){try{
 ...
 const result=await new Promise((resolve,reject)=>{
  ...
  child.stderr.on('data',b=>output+=b);child.on('error',reject);child.on('close',status=>{clearTimeout(timer);fs.writeFileSync(path.join(repo,'test-results/plot-gallery-native.log'),output);resolve({status,output})});
 });
