...
try {
  ...
  const saved = JSON.parse(await read(`slides/${deck.id}/deck.json`));
  eq(saved.slides[0].beats, deck.slides[0].beats, "saved source ownership preserves every authored animation endpoint and source link");
  eq(exported.payload.deck.slides[0].beats, portablePayload({ deck: saved }).deck.slides[0].beats, "portable animation endpoints equal the explicit authoring-data projection");
  eq(/\"(?:svgPath|recipePath|sourcePath)\"/.test(JSON.stringify(exported.payload.deck)), false, "portable deck contains no private source paths");
