...
try {
  for(const engine of ['GUI','Node']) for(const binary of [false,true]) for(const missing of [false,true]) {
    ...
    await fs.symlink(B,path.join(A,'fig','escape'),'dir');
  ...
  {
    const {A,B,bridge}=await fixture('new-publication');await fs.symlink(B,path.join(A,'fig','escape'),'dir');
  ...
  {
    const {A,B,bridge}=await fixture('journal-path');await fs.rm(path.join(A,'.meta'),{recursive:true});await fs.symlink(B,path.join(A,'.meta'),'dir');
  ...
  {
    const {A,bridge}=await fixture('allowed-alias'),alias=path.join(path.dirname(A),'alias');await fs.symlink(A,alias,'dir');await fs.mkdir(path.join(A,'owned'));await fs.symlink(path.join(A,'owned'),path.join(A,'fig','inside'),'dir');
  ...
  {
    const {A,B}=await fixture('export-generated');await fs.mkdir(path.join(A,'paper'));await fs.symlink(B,path.join(A,'paper','escape'),'dir');
  ...
  }
  {
    ...
    try {
      ...
      await fs.symlink(B,path.join(A,'fig','escape'),'dir');await fs.writeFile(path.join(B,'owned.json'),'foreign authored bytes');await fs.writeFile(path.join(A,'project.json'),changed);
