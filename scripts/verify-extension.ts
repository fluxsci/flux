// Pure gate for the "Add to FluxLib" browser extension — everything provable from the files a
// checkout actually contains, with no build and no network.
//
// The centrepiece is the PAGE READER (extension/page.js), which decides what gets captured —
// same job the bookmarklet does, so it gets the same real-publisher fixtures. Around it sit the
// worker's structural guarantees and the capture-name contract.
//
// The build's one-source-of-truth property — dist/ is a faithful copy of the sources, and the
// committed signed .xpi is a faithful copy of dist/ — lives in scripts/verify-extension-build.ts
// on the `bundle` tier. It reads extension/dist, which is gitignored and does not exist until
// `npm run build` runs, so asserting it here made this gate pass or fail on whether the machine
...
// --- 1: the signed add-on a checkout ships -------------------------------------------------
...
//
// Its CONTENTS are compared against a fresh build in verify-extension-build.ts. What is
// checkable here is everything the archive says about itself.
{
...
// — the first signing worked and every one after it would have failed on a duplicate version.
{
  ...
  // That the built manifest actually carries this version is checked post-build, in
