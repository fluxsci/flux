...
try{
 ...
 const exported=new Map([[source,'temporary'],[resource,'owned profile'],[exportJournal,JSON.stringify({version:1,id:'owner',files:[{path:'a.qmd',original:'α\r\n',transformed:'temporary',times:{atimeMs:1,mtimeMs:2}}],generated:[{path:'profile.yml',content:'owned profile'}]})]]);
 const effects:string[]=[];owned=false;
 const exportIO={readText:async(p:string)=>exported.get(p)??null,writeText:async(p:string,t:string)=>{effects.push(p);exported.set(p,t)},removeFile:async(p:string)=>{effects.push('remove:'+p);exported.delete(p)},setTimes:async(p:string)=>{effects.push('times:'+p)},assertOwned:owner};
 await assert.rejects(recoverExportSources(exportIO,root),/revoked/);assert.deepEqual(effects,[]);assert.equal(exported.get(source),'temporary');assert.ok(exported.has(exportJournal)&&exported.has(resource));
 owned=true;await recoverExportSources(exportIO,root);assert.equal(exported.get(source),'α\r\n');assert.ok(!exported.has(exportJournal)&&!exported.has(resource));assert.ok(effects.includes('times:'+source));
